...
internal sealed class MainWindowSmokeTests
{
    public void MainWindowDefaultsToCurrentUserCleanupScope()
    {
        ...
        try
        {
            ...
            Assert(
                ...
                && window.SafetySummaryHeaderAutomationHelpTextValue.Contains("Header state: neutral", StringComparison.OrdinalIgnoreCase)
            ...
            Assert(
                ...
                && window.QuarantineShortlistHeaderAutomationHelpTextValue.Contains("Header state: neutral", StringComparison.OrdinalIgnoreCase)
            ...
            Assert(
                window.RestoreManifestReviewSummaryTextValue.Contains("Manifest review summary", StringComparison.OrdinalIgnoreCase)
                && window.RestoreManifestReviewSummaryTextValue.Contains("waiting for Discover manifests", StringComparison.OrdinalIgnoreCase)
                && window.RestoreManifestReviewSummaryTextValue.Contains("no files are restored", StringComparison.OrdinalIgnoreCase),
                "Startup manifest review summary should point to discovery without implying restore execution.");
            Assert(window.RestoreManifestReviewSummaryStyleValue == "Neutral", "Startup manifest review summary should use neutral styling.");
            Assert(
                window.RestoreManifestReviewSummaryToolTipValue.Contains("Summary state: neutral", StringComparison.OrdinalIgnoreCase)
                && window.RestoreManifestReviewSummaryToolTipValue.Contains("read-only review context", StringComparison.OrdinalIgnoreCase)
                && window.RestoreManifestReviewSummaryToolTipValue.Contains("does not create folders", StringComparison.OrdinalIgnoreCase)
                && window.RestoreManifestReviewSummaryToolTipValue.Contains("approve restore", StringComparison.OrdinalIgnoreCase),
                "Startup manifest review summary tooltip should expose the read-only restore boundary.");
            Assert(
                string.Equals(
                    window.RestoreManifestReviewSummaryAutomationHelpTextValue,
                    window.RestoreManifestReviewSummaryToolTipValue,
                    StringComparison.Ordinal),
                "Manifest review summary automation help text should mirror its tooltip.");
            Assert(
            ...
            Assert(
                window.BrowseQuarantineRootButtonAutomationHelpTextValue.Contains("Quarantine Root", StringComparison.OrdinalIgnoreCase)
                && window.BrowseQuarantineRootButtonAutomationHelpTextValue.Contains("preview paths only", StringComparison.OrdinalIgnoreCase)
                && window.BrowseQuarantineRootButtonAutomationHelpTextValue.Contains("does not create folders", StringComparison.OrdinalIgnoreCase)
                && window.BrowseQuarantineRootButtonAutomationHelpTextValue.Contains("approve cleanup", StringComparison.OrdinalIgnoreCase),
                "Quarantine Root browse automation help text should explain preview-only selection and no-file-modified behavior.");
            Assert(
            ...
            Assert(
                window.BrowseCleanupScopeButtonAutomationHelpTextValue.Contains("Cleanup Scope", StringComparison.OrdinalIgnoreCase)
                && window.BrowseCleanupScopeButtonAutomationHelpTextValue.Contains("does not start a scan", StringComparison.OrdinalIgnoreCase)
                && window.BrowseCleanupScopeButtonAutomationHelpTextValue.Contains("real-profile gate", StringComparison.OrdinalIgnoreCase)
                && window.BrowseCleanupScopeButtonAutomationHelpTextValue.Contains("approve cleanup", StringComparison.OrdinalIgnoreCase),
                "Cleanup Scope browse automation help text should explain path-only selection and scan-gate boundaries.");
            ...
            Assert(
                ...
                "Quarantine preview placeholder should use the visible Preview shortlist quarantine label at startup.");
            Assert(
    ...
    public void MainWindowRunsFixtureReviewInteractionsThroughWpfShell()
    {
        ...
        try
        {
            ...
            AssertQuarantinePreviewStatusHelpText(window, "Inline Quarantine Preview status help text should expose the startup preview boundary.");
            Assert(
            ...
            Assert(
                !window.QuarantinePreviewTextValue.Contains("Execution implemented", StringComparison.OrdinalIgnoreCase),
                "Fixture preview pane should rely on scope-status wording instead of technical implementation flags.");
            Assert(
            ...
            Assert(
                ...
                && window.QuarantineExecutionGateTextValue.Contains("fixture executable", StringComparison.OrdinalIgnoreCase)
                && window.QuarantineExecutionGateTextValue.Contains("Readiness blockers: 0", StringComparison.OrdinalIgnoreCase)
                && window.QuarantineExecutionGateTextValue.Contains("review context only", StringComparison.OrdinalIgnoreCase),
                "Fixture execution gate should include the read-only execution readiness contract.");
            Assert(
            ...
            Assert(
                ...
                && !window.QuarantineExecutionGateTextValue.Contains("Execution implemented", StringComparison.OrdinalIgnoreCase)
                && window.QuarantineExecutionGateTextValue.Contains("Fixture-only execution is available", StringComparison.OrdinalIgnoreCase)
            ...
            Assert(
                ...
                && window.QuarantinePreviewTextValue.Contains("Preview shortlist quarantine", StringComparison.OrdinalIgnoreCase)
                && !window.QuarantinePreviewTextValue.Contains("Preview quarantine.", StringComparison.OrdinalIgnoreCase),
                "Changing the quarantine root should clear stale preview readiness text with the visible preview button label.");
            Assert(
            ...
            Assert(
                ...
                && window.QuarantinePreviewTextValue.Contains("Preview shortlist quarantine", StringComparison.OrdinalIgnoreCase)
                && !window.QuarantinePreviewTextValue.Contains("Preview quarantine.", StringComparison.OrdinalIgnoreCase),
                "Removing the shortlisted row should clear preview readiness text with the visible preview button label.");
            Assert(
    ...
    public void MainWindowExecutesQuarantineForFixtureScopeOnly()
    {
        ...
        try
        {
            ...
            Assert(
                ...
                && window.SafetySummaryHeaderToolTipValue.Contains("Header state: needs review", StringComparison.OrdinalIgnoreCase)
                && window.SafetySummaryHeaderToolTipValue.Contains("read-only review context", StringComparison.OrdinalIgnoreCase)
                && window.SafetySummaryHeaderToolTipValue.Contains("not cleanup approval", StringComparison.OrdinalIgnoreCase),
            ...
            Assert(
                ...
                && window.SafetySummaryHeaderAutomationHelpTextValue.Contains("Header state: needs review", StringComparison.OrdinalIgnoreCase)
                && window.SafetySummaryHeaderAutomationHelpTextValue.Contains("read-only review context", StringComparison.OrdinalIgnoreCase)
                && window.SafetySummaryHeaderAutomationHelpTextValue.Contains("not cleanup approval", StringComparison.OrdinalIgnoreCase),
            ...
            Assert(
                ...
                && window.QuarantineShortlistHeaderToolTipValue.Contains("Header state: ready or completed", StringComparison.OrdinalIgnoreCase)
                && window.QuarantineShortlistHeaderToolTipValue.Contains("read-only review context", StringComparison.OrdinalIgnoreCase)
                && window.QuarantineShortlistHeaderToolTipValue.Contains("not cleanup approval", StringComparison.OrdinalIgnoreCase),
            ...
            Assert(
                !window.QuarantineExecutionGateTextValue.Contains("Execution implemented", StringComparison.OrdinalIgnoreCase)
                && window.QuarantineExecutionGateTextValue.Contains("Fixture-only execution is available", StringComparison.OrdinalIgnoreCase),
                "Fixture execution gate should show execution availability through scope-status wording.");
            Assert(
    ...
    public void MainWindowDiscoversQuarantineManifestsReadOnly()
    {
        ...
        try
        {
            ...
            Assert(
                discoveryWindow.RestoreManifestReviewSummaryTextValue.Contains("1 discovered", StringComparison.OrdinalIgnoreCase)
                && discoveryWindow.RestoreManifestReviewSummaryTextValue.Contains("one selected", StringComparison.OrdinalIgnoreCase)
                && discoveryWindow.RestoreManifestReviewSummaryTextValue.Contains("selected readiness not previewed", StringComparison.OrdinalIgnoreCase)
                && discoveryWindow.RestoreManifestReviewSummaryTextValue.Contains("all-manifest readiness not previewed", StringComparison.OrdinalIgnoreCase)
                && discoveryWindow.RestoreManifestReviewSummaryTextValue.Contains("no files were restored", StringComparison.OrdinalIgnoreCase),
                "Manifest review summary should compactly show discovery state before readiness preview.");
            Assert(discoveryWindow.RestoreManifestReviewSummaryStyleValue == "Information", "Discovered manifest summary should use information styling before blockers or execution.");
            Assert(
            ...
            Assert(
                discoveryWindow.RestoreManifestReviewSummaryTextValue.Contains("selected readiness 1 restorable/0 blocked", StringComparison.OrdinalIgnoreCase)
                && discoveryWindow.RestoreManifestReviewSummaryTextValue.Contains("selected restore gate not previewed", StringComparison.OrdinalIgnoreCase)
                && discoveryWindow.RestoreManifestReviewSummaryToolTipValue.Contains("Summary state: information", StringComparison.OrdinalIgnoreCase)
                && discoveryWindow.RestoreManifestReviewSummaryToolTipValue.Contains("does not create folders", StringComparison.OrdinalIgnoreCase),
                "Manifest review summary should show selected readiness state and preserve read-only help text.");
            ...
            Assert(
                ...
                && discoveryWindow.RestoreManifestReviewSummaryToolTipValue.Contains("Summary state: success", StringComparison.OrdinalIgnoreCase),
                "Manifest review summary should show the open fixture selected restore gate without implying broader restore approval.");
            AssertSelectedRestoreExecutionGateHelpCue(
                discoveryWindow,
            ...
            Assert(
                discoveryWindow.RestoreManifestReviewSummaryTextValue.Contains("all-manifest readiness 1 restorable/0 blocked", StringComparison.OrdinalIgnoreCase)
                && discoveryWindow.RestoreManifestReviewSummaryTextValue.Contains("selected restore gate open", StringComparison.OrdinalIgnoreCase),
                "Manifest review summary should include all-manifest readiness counts after preview.");
    ...
    public void MainWindowKeepsQuarantineExecutionUnavailableForCustomScope()
    {
        ...
        try
        {
            ...
            Assert(
                !window.QuarantinePreviewTextValue.Contains("Execution implemented", StringComparison.OrdinalIgnoreCase),
                "Custom-scope preview should rely on scope-status wording instead of technical implementation flags.");
            Assert(
                window.QuarantinePreviewTextValue.Contains("Execution scope status", StringComparison.OrdinalIgnoreCase)
            ...
            Assert(
                ...
                && window.QuarantinePreviewTextValue.Contains("Scope: custom", StringComparison.OrdinalIgnoreCase)
                && window.QuarantinePreviewTextValue.Contains("Readiness blocker | Scope and policy", StringComparison.OrdinalIgnoreCase)
                && window.QuarantinePreviewTextValue.Contains("Custom non-fixture Cleanup Scopes remain preview-only", StringComparison.OrdinalIgnoreCase),
                "Custom-scope preview should show the readiness contract and custom preview-only blocker.");
            Assert(
                window.QuarantineExecutionGateTextValue.Contains("Execution scope status", StringComparison.OrdinalIgnoreCase)
            ...
            Assert(
                ...
                && window.QuarantineReadinessSummaryTextValue.Contains("custom scope preview-only", StringComparison.OrdinalIgnoreCase)
                && window.QuarantineReadinessSummaryTextValue.Contains("movement unavailable", StringComparison.OrdinalIgnoreCase),
                "Compact readiness summary should keep custom scopes visibly preview-only with custom key-blocker wording after exact QUARANTINE.");
    ...
    public void MainWindowShowsRealProfileChildReadinessContractForSyntheticPreview()
    {
        ...
        try
        {
            ...
            Assert(
                ...
                && window.QuarantineReadinessSummaryTextValue.Contains("Missing: Scope and policy", StringComparison.OrdinalIgnoreCase)
                && window.QuarantineReadinessSummaryTextValue.Contains("Key blockers:", StringComparison.OrdinalIgnoreCase)
                && window.QuarantineReadinessSummaryTextValue.Contains("exact profile scope", StringComparison.OrdinalIgnoreCase),
                "Compact readiness summary should keep real-profile child scopes visibly preview-only with exact-scope key-blocker wording.");
    ...
    private static void AssertQuarantinePreviewStatusHelpText(MainWindow window, string message)
    {
        ...
        Assert(
            ...
            && window.QuarantinePreviewStatusToolTipValue.Contains("restore files", StringComparison.OrdinalIgnoreCase)
            && window.QuarantinePreviewStatusToolTipValue.Contains("delete files", StringComparison.OrdinalIgnoreCase)
            && window.QuarantinePreviewStatusToolTipValue.Contains("approve cleanup", StringComparison.OrdinalIgnoreCase),
            message + " Tooltip should mirror the current status text and safety boundary.");
        Assert(
            ...
            && window.QuarantinePreviewStatusAutomationHelpTextValue.Contains("restore files", StringComparison.OrdinalIgnoreCase)
