...
function Get-FixtureReviewChecklistItems {
    ...
    @(
        ...
        "In the Quarantine tab's Restore Manifest Review panel, use Discover manifests and Preview all-manifest readiness; check the Discover manifests ? help cue, selected manifest ? help cue, all-manifest readiness ? help cue, Restore Manifest review summary states/tooltips, cue/control pairs stay together when rows wrap, no all-manifest restore action wording, and all-manifest readiness scope tooltips.",
        "Use selected manifest readiness and the selected restore gate; check selected-only readiness wording, the highlighted selected-restore key-status strip, selected restore confirmation ? help cue, selected restore Approval boundary, Execution scope status, Selected Restore Execution Gate ? help cue in waiting/closed/open/restored states without crowding the gate area, fixture restore tooltips, and selected restore revalidation evidence for exact real-profile Restore Manifests when available.",
        "Without scanning C:\Users\moxhe as part of this fixture pass, confirm custom and non-exact real-profile Quarantine scopes stay preview-only when using custom preview-only paths or existing synthetic readiness evidence. For exact synthetic real-profile readiness evidence, missing live source evidence should keep Can approve real-profile movement: no through Pre-Execution Revalidation, while ADR 0018 first-phase limits such as 10 rows, 1 GB, no-category, and strict descendant checks stay explicit. For selected restore, custom and non-exact real-profile manifests stay unavailable even with RESTORE; exact real-profile selected restore is a separate user-approved manual trust path under ADR 0019, not part of the fixture pass."
    )
}
...
function New-FixtureAcceptanceNotes {
    ...
    $lines.Add("- Fixture-only Quarantine execution is allowed only inside the synthetic Cleanup Scope; fixture selected restore is allowed in the synthetic Cleanup Scope, while exact real-profile selected restore is a separate user-approved manual trust path.")
    $lines.Add("")
