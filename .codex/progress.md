...
Latest small packet added a source-level WPF guard proving Real-Profile Quarantine Approval Evidence stays display-only: WPF builds it for Quarantine Execution Gate output, leaves current-build movement availability at the default unavailable value, skips fixture scopes, keeps `CanApproveForRealProfileMovement` inside display formatting, and continues enabling `Quarantine included shortlist` only from `_currentQuarantineExecutionGate.CanExecute`.
