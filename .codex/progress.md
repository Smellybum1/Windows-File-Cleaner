...
Latest small packet added the non-moving `RealProfileRestoreReadiness` core model/tests. It consumes selected Restore Manifest review, selected restore confirmation draft, and selected restore execution gate evidence; blocks fixture/custom/child-scope restore evidence, blocked/recovery/not-moved selected readiness, wrong confirmation, and unavailable real-profile selected restore implementation; and lets `QuarantineExecutionReadiness` consume the result while real-profile/custom execution remains unavailable. No WPF gate behavior, scan behavior, real-profile movement, restore, deletion, or cleanup history changed.

## Next recommended work

1. Add WPF readiness output that groups the current core readiness inputs: Quarantine Root Execution Safety, Pre-Execution Revalidation, and Real-Profile Restore Readiness. Keep real-profile/custom execution disabled.
2. Design selected-manifest real-profile Undo Quarantine execution as a later safety packet before forward real-profile movement. Keep real-profile movement blocked.
