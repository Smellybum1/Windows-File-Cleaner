...
Latest small packet surfaced read-only Real-Profile Quarantine Approval Evidence in the WPF Quarantine Execution Gate for non-fixture preview-only scopes. The gate now shows exact `QUARANTINE` matching, exact real-profile scope evidence, readiness blocker presence, current-build movement availability, and `Can approve real-profile movement: no`; fixture gate output stays uncluttered, and real-profile movement remains unavailable.
