...
Latest next-batch review wrapper update: `.\tools\Invoke-RealProfileNextBatchReview.cmd` runs the exact-profile next-batch evidence preset and then prints the manual WPF checklist in one terminal-only flow. The default wrapper path passed full MVP preflight, daily readiness, exact-profile Restore Manifest display, recovery-review and undo-work focus, and checklist printing; `-SkipMvpPreflight` also passed as a smoke check and is not fresh movement evidence. The wrapper does not launch WPF, scan, move, restore, delete, write Restore Manifests, approve cleanup, create cleanup history, create shortcuts, or install anything.
