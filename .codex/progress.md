...
Latest small packet aligned the fixture review checklist and README with the WPF Real-Profile Quarantine Approval Evidence output. The manual review prompt now asks reviewers to confirm the Quarantine Execution Gate shows read-only approval evidence with `Can approve real-profile movement: no` for real-profile/custom preview-only scopes while ADR 0017/0019 blockers stay explicit.
