...
Latest small packet wired the existing non-`D:` Quarantine Root acknowledgement into WPF as read-only readiness evidence. Fully qualified non-`D:` roots now enable a local acknowledgement checkbox, acknowledgement changes clear stale preview/gate state, and WPF passes the acknowledgement into Quarantine Root Execution Safety output without enabling real-profile/custom execution. No selected real-profile restore, real-profile Quarantine execution, file movement, deletion, folder creation, manifest writing, or cleanup history changed.
