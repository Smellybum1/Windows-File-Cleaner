...
Latest small packet wired read-only Pre-Execution Revalidation evidence into WPF Quarantine Preview and Quarantine Execution Gate output. The app now builds revalidation from Quarantine Preview, Quarantine Confirmation Draft, Quarantine Action Draft, and Quarantine Root Execution Safety when preview readiness exists, passes it into Real-Profile Quarantine Execution Readiness, and shows compact revalidation evidence without enabling real-profile/custom movement. Real-Profile Restore Readiness remains unwired WPF evidence and still blocks real-profile execution. No file movement, restore, deletion, folder creation, manifest writing, or cleanup history changed.
