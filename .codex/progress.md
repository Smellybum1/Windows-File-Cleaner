...
Latest small packet wired read-only Quarantine Root Execution Safety evidence into WPF Quarantine Preview and Quarantine Execution Gate output. The app now builds root safety from the Quarantine Action Draft when preview readiness exists, passes it into Real-Profile Quarantine Execution Readiness, and shows compact root-safety evidence without enabling real-profile/custom movement. Pre-Execution Revalidation and Real-Profile Restore Readiness remain unwired WPF evidence and still block real-profile execution. No scan behavior, file movement, restore, deletion, folder creation, manifest writing, or cleanup history changed.
