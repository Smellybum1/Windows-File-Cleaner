...
Latest small packet added core Real-Profile Quarantine Approval Evidence so exact `QUARANTINE` is necessary but not sufficient before any future real-profile movement can be considered. The model is read-only, defaults current-build real-profile movement availability to unavailable, carries readiness blockers forward, rejects custom/child scopes, is covered by the readiness-builder source guard, and passed core tests plus whitespace diff without scanning or modifying real user files.
