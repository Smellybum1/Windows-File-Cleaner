...
Latest exact-profile recovery-review entry evidence: `cmd.exe /c tools\Summarize-RestoreManifests.cmd -CleanupScope "C:\Users\moxhe" -RecoveryReviewOnly -ShowEntries` passed and showed the two displayed recovery-review manifests are failed-only NVIDIA `DXCache` directory attempts with moved count `0`, undo work `no`, and recovery review `yes`. `quarantine-action-draft-20260601110130-9010b9d9` failed because a descendant `.nvph` file was in use by another process; `quarantine-action-draft-20260601103558-b3e00fc2` failed because source and destination roots differed before the guarded cross-volume fallback existed. `cmd.exe /c tools\Summarize-RestoreManifests.cmd -CleanupScope "C:\Users\moxhe" -UndoWorkOnly` showed zero exact-profile matches, and `cmd.exe /c tools\Summarize-RestoreManifests.cmd -CleanupScope "C:\Users\moxhe" -RequireAnyDisplayed -RequireNoDisplayedUndoWork` passed with 4 exact-profile displayed manifests, displayed undo work `0`, and displayed recovery review `2`. This was read-only terminal evidence only and did not launch WPF, scan, move, restore, delete, approve cleanup, write Restore Manifests, or create cleanup history.

Latest current-head daily readiness after recovery evidence: `cmd.exe /c tools\Invoke-DailyLocalReadiness.cmd -IncludeFixtureAcceptanceNotes -CleanupScope "C:\Users\moxhe" -RequireAnyDisplayedRestoreManifest -RequireNoDisplayedUndoWork` passed on current `main` at `5b2a982`. It verified completed accepted package notes, printed optional Fixture Acceptance Notes status, ran one accepted package verifier pass with the expected accepted-package/current-HEAD warning (`bc9b869` accepted package versus `5b2a982` current `HEAD`), printed accepted normal/fixture launch commands in print-only mode, and showed exact-profile Restore Manifest display `(4 of 10)`, displayed undo work `0`, and displayed recovery review `2`. No WPF app was launched, no scan was started, and no files were moved, restored, deleted, approved, written to Restore Manifests, or added to cleanup history.

Latest shortcut/installer deferral decision: ADR 0020 keeps accepted package launch commands as the v1 daily path and defers installed shortcut or installer automation to a later explicit user-approved packaging packet. The packet added domain/glossary wording for Installed Shortcut and Installer as future-only concepts, updated README/handoff/roadmap guidance, and did not create shortcuts, install anything, publish a package, launch WPF, scan, move, restore, delete, approve cleanup, write Restore Manifests, or create cleanup history.

Latest current-head next-batch review evidence: `cmd.exe /c tools\Invoke-RealProfileNextBatchReview.cmd` passed on current `main` at `c7cb545`. Full MVP preflight restored, built, ran core tests, ran WPF app tests, ran fixture `-WhatIf`, printed the fixture checklist, and ran whitespace diff checking. Daily readiness verified completed accepted package notes, printed optional Fixture Acceptance Notes status, ran one accepted package verifier pass with the expected package/current-HEAD warning (`bc9b869` accepted package versus `c7cb545` current `HEAD`), printed accepted normal/fixture launch commands in print-only mode, and showed exact-profile Restore Manifest display `(4 of 10)` with displayed undo work `0` and displayed recovery review `2`. Focused recovery-review evidence showed the two older failed exact-profile NVIDIA `DXCache` attempts, focused undo-work evidence showed zero exact-profile matches, and the manual WPF checklist printed. No WPF app was launched, no real-profile scan was started, and no files were moved, restored, deleted, approved, written to Restore Manifests, or added to cleanup history.
...
Latest next-batch WPF checklist update: `.\tools\Show-RealProfileNextBatchChecklist.cmd` prints the recommended combined review wrapper, the separate next-batch evidence preset, and the manual WPF review checklist. It names exact `C:\Users\moxhe` scope, accepted package launch-command printing, Fixture Acceptance Notes status handling, scan/shortlist review, 10-row/1 GB caps, hard blockers, Quarantine tab evidence, the human-only exact `QUARANTINE` click boundary, and rediscover/rescan follow-up without launching WPF, scanning, moving, restoring, deleting, approval, manifest writes, or cleanup history.

Latest next-batch review wrapper update: `.\tools\Invoke-RealProfileNextBatchReview.cmd` runs the exact-profile next-batch evidence preset and then prints the manual WPF checklist in one terminal-only flow. The default wrapper path passed full MVP preflight, daily readiness, exact-profile Restore Manifest display, recovery-review and undo-work focus, and checklist printing; the latest `-SkipMvpPreflight` smoke also passed after the checklist began pointing back to the wrapper, and skipped preflight is not fresh movement evidence. The wrapper does not launch WPF, scan, move, restore, delete, write Restore Manifests, approve cleanup, create cleanup history, create shortcuts, or install anything.
...
Latest daily readiness evidence refresh: `cmd.exe /c tools\Invoke-DailyLocalReadiness.cmd -IncludeFixtureAcceptanceNotes -CleanupScope "C:\Users\moxhe" -RequireAnyDisplayedRestoreManifest -RequireNoDisplayedUndoWork` passed on current `main` at `9dc5b98` after the accepted-launcher output boundary. It verified completed accepted package notes, printed optional Fixture Acceptance Notes status, verified the accepted package once with the expected accepted-package/current-HEAD warning, printed accepted normal and fixture launch commands in print-only mode, and showed the new accepted-release daily-path/debug-shortcut boundary in both accepted launcher sections. Exact-profile displayed Restore Manifest evidence still showed 4 displayed exact-profile manifests, 0 displayed undo-work manifests, and 2 displayed recovery-review manifests. No WPF app was launched, no scan was started, and no files were moved, restored, deleted, approved, written to Restore Manifests, or added to cleanup history.
...
Fresh `.\tools\Invoke-RealProfileNextBatchReview.cmd` evidence has passed on current `main` at `c7cb545`, so the next live-product step can be the user's manual WPF next-batch review if they are ready. Use `.\tools\Invoke-RealProfileQuarantineReadiness.cmd -RequireNextBatchEvidence` when only evidence is needed; it focuses Restore Manifest display rows to exact `C:\Users\moxhe`, includes formal fixture-notes status, proves the exact-profile display exists, and requires zero displayed undo-work manifests. Use `.\tools\Show-RealProfileNextBatchChecklist.cmd` when only the post-preset manual WPF checklist is needed; it now repeats the formal notes handling boundary before launch. Use `.\tools\Invoke-RealProfileQuarantineReadiness.cmd -AllCleanupScopes` only when fixture manifests should also be displayed outside the preset. Use `.\tools\Summarize-RestoreManifests.cmd -CleanupScope "C:\Users\moxhe" -RequireAnyDisplayed -RequireNoDisplayedUndoWork` for direct exact-profile displayed undo-work evidence, and `.\tools\Summarize-RestoreManifests.cmd -CleanupScope "C:\Users\moxhe" -RecoveryReviewOnly -ShowEntries` when exact real-profile recovery-review debt needs inspection. These commands are evidence-only; WPF readiness, exact `QUARANTINE`, Real-Profile Quarantine Approval Evidence, immediate Pre-Execution Revalidation, and explicit user approval for the specific click still remain required.

1. Next safest live-product step is the user's manual WPF next-batch review using the printed checklist, if they are ready. Do not click Quarantine unless the user explicitly chooses a specific tiny exact `C:\Users\moxhe` batch after WPF readiness, exact `QUARANTINE`, Real-Profile Quarantine Approval Evidence, and immediate Pre-Execution Revalidation are visible. Reuse `.\tools\Invoke-DailyLocalReadiness.cmd` when accepted package and Restore Manifest evidence should be refreshed, add `-IncludeFixtureAcceptanceNotes` when formal fixture notes status should be visible, use `.\tools\Start-AcceptedLocalRelease.cmd -PrintOnly` when only the accepted package launch command is needed, complete formal fixture acceptance notes if desired, or rerun `.\tools\Invoke-RealProfileNextBatchReview.cmd` if evidence becomes stale. Treat any debug-build desktop shortcut as development-only context, not accepted-package evidence or the recommended daily path; ADR 0020 defers installed shortcut or installer automation until a later explicit user-approved packaging packet. The first-live selected restore recovery loop is complete by user report: rediscovery showed restored/already restored, rescan completed normally, and the restored `pip\cache\http\b\c` path appeared again. Keep future movement selected-batch-only, exact `C:\Users\moxhe`, exact confirmation, immediate revalidation, no broad/all-manifest Undo, no permanent deletion, and no cleanup history.
...
3. If formal fixture acceptance notes are useful before movement work, first run `.\tools\Summarize-FixtureAcceptanceNotes.cmd -Path ".local\fixture-review-acceptance\fixture-acceptance-20260601-132126.md"` or the strict `-RequireComplete` variant to see the current blockers and exact recorder command. Run that printed `Record-FixtureAcceptanceNotes.cmd -Path ... -RecordManualAcceptance` command only after an actual all-pass visible fixture review; fill notes manually instead when there were issues or not-checked items. The user has already reported that the visible fixture pass looks good, but the local checklist remains unfilled.
4. Revisit .NET 10, permanent deletion, persisted cleanup history, broad restore, and any ADR 0020 follow-up shortcut/installer automation only as separate future decisions.
...
- Completed accepted package notes were verified for `.local\releases\windows-file-cleaner-v20260602-011556` at accepted package commit `bc9b869`.
- Optional Fixture Acceptance Notes status printed and confirmed `.local\fixture-review-acceptance\fixture-acceptance-20260601-132126.md` remains formally unfilled.
...
- Added a feature evidence note and refreshed the next-batch wrapper, roadmap, handoff, and progress evidence wording.
- Did not launch WPF, scan `C:\Users\moxhe`, move, restore, delete, approve cleanup, write Restore Manifests, create shortcuts, install anything, or create cleanup history.
...
- Whether the user wants to do the manual WPF next-batch review now that fresh terminal evidence has passed.

Follow-up work:

- Ask the user to do the manual WPF next-batch review if they are ready.
- Keep any next batch tiny, exact-profile, readiness-gated, selected-batch-only, and user-clicked.

Risky assumptions:

- The accepted-package/current-HEAD warning remains expected while the accepted package is behind newer docs-only commits.

### 2026-06-02: Shortcut Installer Deferral Decision

Status: completed

Evidence:

- ADR 0020 accepted that accepted package launch commands remain the v1 daily path.
- Installed shortcut and installer automation are deferred until a later explicit user-approved packaging packet defines target selection, stale-target handling, created artifacts, removal behavior, and read-only verification.
- Debug-build shortcuts remain development-only context, not accepted-package evidence or the recommended daily path.

Implementation:

- Added ADR 0020 and a feature completion note.
- Updated README Daily Local Use and release guidance.
- Added future-only glossary rows for Installed Shortcut and Installer.
- Updated the Portable Release Package domain rule, live-product roadmap, handoff, and progress docs.
- Did not create shortcuts, install anything, publish a package, launch WPF, scan, move, restore, delete, approve cleanup, write Restore Manifests, or create cleanup history.
...
- Whether a future ADR 0020 follow-up should add an installed shortcut or installer after the user explicitly asks for that packaging work.

Follow-up work:

- Keep using daily readiness and accepted package print-only commands until a later shortcut/installer packet exists.

Risky assumptions:

- Accepted package launch commands are good enough for daily local use while live-product safety is still maturing.

### 2026-06-02: Current-Head Daily Readiness After Recovery Evidence
...
- Completed accepted package notes were verified for `.local\releases\windows-file-cleaner-v20260602-011556` at accepted package commit `bc9b869`.
- Optional Fixture Acceptance Notes status printed and confirmed `.local\fixture-review-acceptance\fixture-acceptance-20260601-132126.md` remains formally unfilled.
- Accepted package verification passed with the expected accepted-package/current-HEAD warning (`bc9b869` package versus `5b2a982` current `HEAD`).
- Accepted normal and fixture launch commands printed in print-only mode.
- Exact-profile Restore Manifest display showed 4 of 10 manifests, displayed undo work `0`, and displayed recovery review `2`.

Implementation:

- Added a feature note and refreshed handoff/progress/roadmap evidence wording.
...
- Whether a future user-approved packaging packet should add an installed shortcut or installer.

Follow-up work:

- Continue using the daily readiness command when accepted-package and exact-profile Restore Manifest evidence should be refreshed.
...
### 2026-06-02: Exact-Profile Recovery Review Entry Evidence

Status: completed

Evidence:

- `cmd.exe /c tools\Summarize-RestoreManifests.cmd -CleanupScope "C:\Users\moxhe" -RecoveryReviewOnly -ShowEntries` passed and displayed 2 of 10 manifests needing exact-profile recovery review.
- Both displayed manifests were failed-only NVIDIA `DXCache` directory attempts with moved count `0`, undo work `no`, and recovery review `yes`.
...
- `cmd.exe /c tools\Summarize-RestoreManifests.cmd -CleanupScope "C:\Users\moxhe" -UndoWorkOnly` showed zero exact-profile matches.
- `cmd.exe /c tools\Summarize-RestoreManifests.cmd -CleanupScope "C:\Users\moxhe" -RequireAnyDisplayed -RequireNoDisplayedUndoWork` passed with 4 exact-profile displayed manifests, displayed undo work `0`, and displayed recovery review `2`.
...
- Whether a future WPF restore/history surface should group recovery-review manifests for easier review.

Follow-up work:

- Refresh `-RecoveryReviewOnly -ShowEntries` or exact-profile undo-work evidence before future movement review when manifest state may have changed.

Risky assumptions:

- The two failed NVIDIA `DXCache` manifests should remain visible recovery-review evidence rather than being hidden or automatically cleaned up.
...
- The next-batch evidence preset already prints Fixture Acceptance Notes status.
- Incomplete fixture-note summaries now print the exact recorder command, but the recorder remains only for after an actual all-pass visible fixture review.
- The standalone WPF checklist is the last terminal guidance before a future human-run accepted package review, so it should repeat the incomplete-notes boundary.

Implementation:

- Added one `Before launch` checklist item to `tools\Show-RealProfileNextBatchChecklist.ps1`.
- The new item tells the reviewer to confirm Fixture Acceptance Notes status from the evidence preset, review printed summary guidance when formal notes are incomplete, record notes only after an actual all-pass visible fixture review, and fill notes manually for issues/not-checked items.
- Did not launch WPF, scan, move, restore, delete, approve cleanup, write Restore Manifests, or create cleanup history.

Verification:

- `cmd.exe /c tools\Show-RealProfileNextBatchChecklist.cmd`
- `cmd.exe /c tools\Invoke-RealProfileNextBatchReview.cmd -SkipMvpPreflight`
- `rg -n "Fixture Acceptance Notes status|actual all-pass visible fixture review|Next-Batch Checklist Fixture Notes Guidance|Record-FixtureAcceptanceNotes" tools docs README.md .codex`
- `git diff --check`
...
- Whether the user wants to run another visible fixture pass and record formal all-pass notes, or keep current user-reported visual acceptance as informal evidence.

Follow-up work:

- Keep `-RequireFixtureAcceptanceComplete` optional unless a future review explicitly makes formal fixture notes strict.

Risky assumptions:

- Repeating the fixture-notes recorder boundary in the next-batch checklist reduces review friction without implying automatic fixture acceptance.

### 2026-06-02: Fixture Acceptance Summary Recorder Guidance

Status: completed

Evidence:

- The latest real fixture notes `.local\fixture-review-acceptance\fixture-acceptance-20260601-132126.md` remain incomplete: preflight/worktree evidence not recorded, overall result not recorded, and 10 checklist items not recorded.
- `Record-FixtureAcceptanceNotes.cmd` is still intentionally explicit and should be used only after an actual all-pass visible fixture review.

Implementation:

- Updated `Summarize-FixtureAcceptanceNotes.ps1` so incomplete summary output prints the exact recorder command for the summarized notes file.
- Incomplete `-RequireComplete` output now prints the same guidance before exiting nonzero.
- The guidance says to use the recorder only after an actual all-pass visible fixture review and to fill notes manually when there were issues or not-checked items.
- Did not record fixture notes automatically, launch WPF, create fixtures, scan, move, restore, delete, approve cleanup, write Restore Manifests, or create cleanup history.
...
- `cmd.exe /c tools\Summarize-FixtureAcceptanceNotes.cmd -Path ".local\fixture-review-acceptance\fixture-acceptance-20260601-132126.md" -RequireComplete` failed as expected and printed the recorder guidance before exit.
- `git diff --check`
...
- No ADR added. This is terminal guidance for existing ignored local notes tooling.

Open questions:

- Whether the user wants to run the visible fixture pass again and then record formal all-pass notes, or leave the existing user-reported visual acceptance as informal evidence.

Follow-up work:

- Re-run the summary or strict completion check when formal fixture notes evidence matters before a future movement review.

Risky assumptions:

- Printing the exact recorder command near the incomplete-notes blocker reduces friction without weakening the manual acceptance boundary.

### 2026-06-02: Daily Readiness Evidence Refresh After Launcher Boundary
...
- The output verified completed accepted package notes, printed optional Fixture Acceptance Notes status, verified the accepted package once with the expected accepted-package/current-HEAD warning, printed accepted normal and fixture launch commands in print-only mode, and repeated the accepted-release daily-path/debug-shortcut boundary.
- Exact-profile Restore Manifest display stayed at 4 displayed manifests, 0 displayed undo-work manifests, and 2 displayed recovery-review manifests.

Implementation:

- Updated handoff/progress and daily readiness feature notes with fresh read-only evidence only.
- Did not change app code, package artifacts, shortcuts, installer behavior, WPF launch behavior, scan behavior, movement, restore, deletion, approval, manifest writes, or cleanup history.
...
- No ADR added. This is evidence refresh documentation for existing read-only tooling.

Open questions:

- Whether later shortcut or installer automation is worth an explicit user-approved packaging packet.

Follow-up work:

- Reuse the daily readiness wrapper before manual accepted-package launches or before a future next-batch readiness review.
- Keep any real-profile movement human-clicked, exact-scope, exact-confirmed, and tied to fresh readiness evidence.

Risky assumptions:

- Recording the fresh evidence in tracked docs is useful even though the accepted package itself remains intentionally older than current docs/tooling commits.
...
- The README Daily Local Use section already points ordinary local review at the accepted Portable Release Package launcher.
- The handoff still listed a desktop shortcut target under the debug build output, which could confuse future daily-use guidance.
- Existing release/launcher docs explicitly defer installed shortcut or installer automation to a later user-approved packaging packet.

Implementation:

- Clarified README Daily Local Use that debug-build desktop shortcuts are development conveniences only.
- Updated the handoff current-state and startup prompt so accepted package print commands are the daily launch-command path.
- Added this feature note and progress entry.
- Did not create, modify, or delete shortcuts; did not publish or accept a new package; did not launch WPF; did not scan, move, restore, delete, approve cleanup, write Restore Manifests, or create cleanup history.
...
- No ADR added. This is launch-path documentation clarity for existing portable package tooling, not a durable architecture, persistence, cleanup execution, restore scope, deployment model, data model, or security change.
...
### 2026-06-02: Current-Head Daily Readiness Evidence Refresh

Status: completed

Evidence:

- Current `main` is `0bc2e96`.
...
- The current accepted-package verifier warning is expected because the accepted package predates newer docs/tooling commits.
...
- Updated handoff/progress and the daily readiness feature notes with current-HEAD evidence only.
...
- `cmd.exe /c tools\Invoke-DailyLocalReadiness.cmd -IncludeFixtureAcceptanceNotes -CleanupScope "C:\Users\moxhe" -RequireAnyDisplayedRestoreManifest -RequireNoDisplayedUndoWork` passed. It verified completed accepted package notes, printed optional Fixture Acceptance Notes status, ran one accepted package verifier pass, printed accepted normal and fixture launch commands in print-only mode, and showed exact-profile Restore Manifest display `(4 of 10)` with displayed undo work `0` and displayed recovery review `2`. The expected accepted-package/current-HEAD warning appeared because the accepted package predates newer docs/tooling commits.
...
- `cmd.exe /c tools\Summarize-RestoreManifests.cmd -CleanupScope "C:\Users\moxhe" -UndoWorkOnly` passed and showed zero exact-profile undo-work manifests.
- `cmd.exe /c tools\Summarize-LocalReleaseAcceptanceNotes.cmd -RequireComplete` passed and confirmed the accepted package notes remain complete.
...
- Before any future tiny exact real-profile Quarantine batch, run full `Invoke-RealProfileQuarantineReadiness.cmd` or `Invoke-RealProfileNextBatchReview.cmd`, then have the user review WPF readiness, exact `QUARANTINE`, approval evidence, and immediate revalidation before any specific click.

Risky assumptions:

- Current read-only terminal evidence is useful enough to refresh handoff/progress without cutting a new accepted package.

### 2026-06-02: Real-Profile Next-Batch Checklist Wrapper Hint

Status: completed

Evidence:

- The standalone next-batch WPF checklist was still safe, but it only named the separate readiness preset and accepted package printer, while the newer combined review wrapper is the preferred path when evidence and checklist should stay together.
- The safe change was output/docs polish only: surface the combined wrapper from the checklist without launching WPF or touching real-profile files.

Implementation:

- Updated `tools\Show-RealProfileNextBatchChecklist.ps1` to print `.\tools\Invoke-RealProfileNextBatchReview.cmd` as the recommended combined terminal review before the separate evidence preset.
- Updated README, the checklist and wrapper feature briefs, handoff, and progress notes.
- Kept all movement boundaries unchanged: no WPF launch, no scan, no move, no restore, no delete, no approval, no Restore Manifest write, no cleanup history, no shortcut, and no installer behavior.

Verification:

- `cmd.exe /c tools\Show-RealProfileNextBatchChecklist.cmd` passed and printed the recommended wrapper command, separate evidence preset, accepted package print command, manual WPF checklist, and terminal-only boundary without launching WPF or touching files.
- `cmd.exe /c tools\Invoke-RealProfileNextBatchReview.cmd -SkipMvpPreflight` passed after the wrapper hint change; it ran the exact-profile evidence preset with skipped-preflight warning and printed the updated checklist without launching WPF, scanning, moving, restoring, deleting, approval, manifest writes, or cleanup history.
...
- No ADR added. This is terminal output and documentation polish over existing ADR 0017/0018/0019 boundaries.
...
- Do not make the checklist launch the wrapper or WPF; it remains terminal guidance only.

### 2026-06-02: Real-Profile Next-Batch Review Wrapper

Status: completed

Evidence:

- The next-batch evidence preset and WPF checklist were safe but separate terminal commands.
- A future human-clicked tiny exact batch benefits from a single terminal path that runs the exact-profile preset first, then prints the manual checklist.

Implementation:

- Added `tools\Invoke-RealProfileNextBatchReview.ps1` and `.cmd`.
- The wrapper runs `Invoke-RealProfileQuarantineReadiness.cmd -RequireNextBatchEvidence`, forwards local evidence-path/root display options, allows `-SkipMvpPreflight` only with warning text, and prints `Show-RealProfileNextBatchChecklist.cmd` only after the preset passes.
- Updated README, domain context, glossary, live-product roadmap, handoff, and this progress log.

Verification:

- `cmd.exe /c tools\Invoke-RealProfileNextBatchReview.cmd` passed. It ran full MVP preflight, including restore, build, core tests, WPF app tests, fixture `-WhatIf`, fixture checklist-only output, and whitespace diff checking, then ran daily readiness, exact-profile Restore Manifest display, recovery-review and undo-work focus, and printed the manual WPF checklist. No WPF app was launched, no real-profile scan was started, and no files were moved, restored, deleted, approved, or added to cleanup history.
- `cmd.exe /c tools\Invoke-RealProfileNextBatchReview.cmd -SkipMvpPreflight` passed. It ran the preset with skipped-preflight warning, printed optional Fixture Acceptance Notes status, showed exact-profile displayed Restore Manifest evidence with zero displayed undo-work manifests, printed recovery-review and undo-work focus, then printed the manual WPF checklist. No WPF app was launched, no scan was started, and no files were moved, restored, deleted, approved, or added to cleanup history.
...
- No ADR added. This is a read-only local tooling wrapper over existing ADR 0017/0018/0019 gates.

Open questions:

- Whether a shortcut or installer is useful later remains a separate packaging decision.

Rejected ideas buffer:

- Do not expose `-AllCleanupScopes` or custom `-CleanupScope` on the next-batch wrapper; use the lower-level readiness command only when intentionally running custom evidence outside the preset.
