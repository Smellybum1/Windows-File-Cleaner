...
Latest small packet added concise key-blocker labels to the compact Quarantine Readiness Summary for preview-only scopes. Synthetic exact real-profile WPF coverage now proves the summary can show ADR 0018 first-phase blockers such as `10-row cap`, `1 GB cap`, `no-category rows`, and `strict descendant checks` while keeping real-profile movement unavailable.
