...
| Undo Quarantine execution | ADR 0008, ADR 0010, ADR 0011, ADR 0012, ADR 0013, Restore Manifest docs, core fixture tests, WPF fixture tests | Current fixture verified, read-only discovery/selection/readiness verified, broad WPF undo out of MVP | Core Undo Quarantine can restore fixture Moved entries, WPF can undo the current fixture execution, discover action-scoped Restore Manifests read-only, select one discovered Restore Manifest for read-only review, and preview restore readiness without restoring files. Restoring selected discovered older manifests and real-profile WPF undo remain future work. |
...
13. After a fixture execution, use `Discover manifests`, select one Restore Manifest, and use `Preview selected readiness`.
