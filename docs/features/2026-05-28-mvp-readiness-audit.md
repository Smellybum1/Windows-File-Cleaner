...
| Undo Quarantine execution | ADR 0008, ADR 0010, ADR 0011, ADR 0012, ADR 0013, ADR 0014, Restore Manifest docs, core fixture tests, WPF fixture tests | Current fixture verified, read-only discovery/selection/confirmation/readiness verified, broad WPF undo out of MVP | Core Undo Quarantine can restore fixture Moved entries, WPF can undo the current fixture execution, discover action-scoped Restore Manifests read-only, select one discovered Restore Manifest for read-only review, preview selected restore confirmation with exact `RESTORE` text while execution remains unavailable, and preview restore readiness without restoring files. Restoring selected discovered older manifests and real-profile WPF undo remain future work. |
...
- `dotnet run --project tests\WindowsFileCleaner.App.Tests\WindowsFileCleaner.App.Tests.csproj --no-build` passed with `All WindowsFileCleaner.App.Tests checks passed.` The app smoke tests now cover WPF shell construction, wrapping review toolbar structure, fixture scan state, review filters, safety shortcuts, Review Shortlist, Quarantine Preview state, fixture execution/undo, read-only manifest discovery, read-only selected manifest review, read-only selected restore confirmation gate, and read-only restore-readiness preview.
...
13. After a fixture execution, use `Discover manifests`, select one Restore Manifest, use `Preview selected readiness`, use `Preview selected restore gate`, and type `RESTORE`; confirm `Can execute: no`.
