...
| Fixture-based verification before real scan | `tests/WindowsFileCleaner.Tests/Program.cs`, `tests/WindowsFileCleaner.App.Tests/Program.cs`, WPF fixture smoke launch docs, progress log | Verified in current repo | Test harnesses cover scanner, classifier, summaries, preview, drafts, CSV, safety guard behavior, WPF shell startup state, and WPF fixture scan state. The WPF app can also launch with a synthetic Cleanup Scope via `--scope`. |
...
| Latest WPF UI retest | User screenshot, WPF shell and fixture scan smoke tests, README manual checklist | Partially automated, pending manual verification | Shell startup, launch-scope wiring, and fixture scan state are automated. The first scan was manually verified, but later filters, shortlist, preview export, and readiness UI still need a fresh visible app run. Use the fixture smoke path first, then retest `C:\Users\moxhe`. |
...
- `dotnet run --project tests\WindowsFileCleaner.App.Tests\WindowsFileCleaner.App.Tests.csproj --no-build` passed with `All WindowsFileCleaner.App.Tests checks passed.`
