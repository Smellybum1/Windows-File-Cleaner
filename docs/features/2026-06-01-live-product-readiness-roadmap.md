...
| Release/packaging readiness | Portable v1 packaging exists through `tools\Publish-LocalRelease.cmd`. It runs MVP preflight by default, publishes a self-contained `Release` / `win-x64` WPF app under ignored `.local\releases\windows-file-cleaner-vYYYYMMDD-HHMMSS\app`, writes local release metadata, creates a zip beside the folder, prints launch commands, and writes release-local normal/fixture launch `.cmd` scripts. `tools\Test-LocalRelease.cmd` can verify the latest or explicit ignored package folder, metadata, launch scripts, zip, and safety-boundary lines without launching WPF or scanning anything. | Completed for a local portable v1 package plus read-only package verification. | Use the portable package for local daily use; use `Test-LocalRelease.cmd -RequireCurrentCommit` after cutting a new package when the package must exactly match current `HEAD`; consider installed shortcut/installer automation only in a later packet. |
