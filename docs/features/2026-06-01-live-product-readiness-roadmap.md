...
| Release/packaging readiness | Portable v1 packaging exists through `tools\Publish-LocalRelease.cmd`. It runs MVP preflight by default, publishes a self-contained `Release` / `win-x64` WPF app under ignored `.local\releases\windows-file-cleaner-vYYYYMMDD-HHMMSS\app`, writes local release metadata, creates a zip beside the folder, and prints launch commands. | Completed for a local portable v1 package. | Use the portable package for local daily use; consider shortcut/installer automation only in a later packet. |
