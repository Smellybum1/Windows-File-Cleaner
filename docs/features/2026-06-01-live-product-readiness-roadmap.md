...
| Release/packaging readiness | Portable v1 packaging exists through `tools\Publish-LocalRelease.cmd`. It runs MVP preflight by default, publishes a self-contained `Release` / `win-x64` WPF app under ignored `.local\releases\windows-file-cleaner-vYYYYMMDD-HHMMSS\app`, writes local release metadata, creates a zip, prints launch commands, and writes release-local normal/fixture launch `.cmd` scripts. `tools\Test-LocalRelease.cmd` can verify the latest or explicit ignored package folder, metadata, launch scripts, zip, and safety-boundary lines without launching WPF or scanning anything. `tools\Start-LocalRelease.cmd` can verify then print or start the latest package from the repo root; `-Fixture` only prefills the repo-local smoke fixture Cleanup Scope. | Completed for a local portable v1 package plus read-only package verification and repo-level launch ergonomics. | Use the portable package for local daily use; use `Test-LocalRelease.cmd -RequireCurrentCommit` after cutting a new package when the package must exactly match current `HEAD`; use `Start-LocalRelease.cmd -PrintOnly -RequireCurrentCommit` to print the current launch command without opening WPF; consider installed shortcut/installer automation only in a later packet. |
