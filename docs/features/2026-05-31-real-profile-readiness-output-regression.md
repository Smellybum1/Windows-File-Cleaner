...
- read-only Quarantine Root Execution Safety, Pre-Execution Revalidation, and Real-Profile Restore Readiness evidence are consumed when available while current-build real-profile execution remains unavailable,
...
- Later WPF root-safety and revalidation evidence wiring updated the test to assert Quarantine Root Execution Safety and Pre-Execution Revalidation are checked. Later WPF restore-readiness evidence wiring added selected-restore gate coverage for synthetic real-profile manifests.
...
- WPF now wires read-only Quarantine Root Execution Safety, Pre-Execution Revalidation, and Real-Profile Restore Readiness evidence. Later packets can implement selected real-profile restore only after explicit approval.
