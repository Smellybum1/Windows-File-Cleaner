...
- read-only Quarantine Root Execution Safety and Pre-Execution Revalidation evidence are consumed when available, while missing Real-Profile Restore Readiness remains grouped visibly,
...
- Later WPF root-safety and revalidation evidence wiring updated the test to assert Quarantine Root Execution Safety and Pre-Execution Revalidation are checked while Real-Profile Restore Readiness remains grouped.
...
- WPF now wires read-only Quarantine Root Execution Safety and Pre-Execution Revalidation evidence. Later packets can wire Real-Profile Restore Readiness evidence without enabling movement.
