...
| Bloat Category Filter | Read-only category lens applied to Storage Scan results. | `StorageCategorySummaryEntry`, optional `BloatCategory` filter | Combines with the active Storage Review Filter. A row may match multiple categories. |
