...
| Pre-Execution Revalidation | Immediate live-filesystem recheck after approval and before a future real-profile Quarantine move. | `PreExecutionRevalidation`, `PreExecutionRevalidationBuilder` | Not a background scan or approval bypass. Current core model is read-only and not wired to WPF execution. Blocks movement when reviewed source, destination, manifest, action, or root evidence is stale or unsafe. |
