...
| Storage Scan Safety Summary | Read-only health readout for safety-relevant scan signals. | `StorageScanSafetySummary`, `StorageScanSafetySummaryBuilder`, `StorageScanSafetyShortcut`, `StorageScanSafetyShortcutFilterBuilder` | Includes bounded access issue examples when present. Not a safety guarantee, cleanup approval, or Cleanup Action. |
