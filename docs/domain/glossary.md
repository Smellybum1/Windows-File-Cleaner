...
| Quarantine Root Execution Safety | Future execution-specific validation of the Quarantine Root Selection before a cleanup action can move files. | Future `QuarantineRootExecutionSafety` | Separate from preview-only Quarantine Root Safety Note. Checks root/scope containment, preferred drive policy, capacity, action-root collisions, and item destination collisions before movement. `D:` is default/preferred; safe non-`D:` roots require extra acknowledgement. Unsafe roots have no override. |
