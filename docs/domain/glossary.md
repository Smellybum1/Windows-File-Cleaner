...
| Selected Restore Manifest Review | Read-only review of one discovered Restore Manifest and its restore readiness. | `SelectedRestoreManifestReview`, `SelectedRestoreManifestReviewBuilder`, `RestoreManifestSelectionBox`, `PreviewSelectedRestoreManifestReadinessButton` | Selection is not approval to restore. Does not call Undo Quarantine execution, write manifests, create folders, move files, delete files, or clean up folders. WPF tooltip and automation help text should keep selected-only/not-approval scope visible, including while the control is disabled. |
