...
| Selected Restore Execution Gate | Read-only gate combining selected restore confirmation, typed text, and execution availability. | `SelectedRestoreExecutionGate`, `SelectedRestoreExecutionGateBuilder`, `SelectedRestoreConfirmationBox` | WPF keeps execution unavailable in the current build; no restore button is exposed. |
