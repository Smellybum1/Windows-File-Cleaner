...
| Selected Row Contents Context | Read-only contained file and folder counts for Storage Scan review rows. | `StorageEntryRow.Contents`, `ContainedFileCount`, `ContainedFolderCount` | Shown in the grid, selected-row detail, and CSV export. Folder count means descendant folders, excluding the selected folder itself. Counts are triage context, not storage savings. |
