...
| Real-Profile Quarantine Execution Readiness | Future composite readiness result required before real-profile WPF Quarantine execution can move files. | Future `QuarantineExecutionReadiness` | Not a single implementation flag. Must combine scope eligibility, review readiness, Quarantine Root Execution Safety, Pre-Execution Revalidation, Real-Profile Restore Readiness, and explicit real-profile approval semantics. |
| Quarantine Root Execution Safety | Future execution-specific validation of the Quarantine Root Selection before a cleanup action can move files. | Future `QuarantineRootExecutionSafety` | Separate from preview-only Quarantine Root Safety Note. Checks root/scope containment, preferred drive policy, capacity, action-root collisions, and item destination collisions before movement. |
