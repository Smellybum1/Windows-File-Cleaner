...
| Quarantine Root Selection | Read-only preview setting for the destination root used when building Quarantine Preview paths. | `QuarantineRootBox`, `BrowseQuarantineRootButton`, `CurrentQuarantineRootPath` | Defaults to `D:\WindowsFileCleanerQuarantine`. Can be typed or browsed. Changing it clears stale previews. Does not create folders or execute Quarantine. |
