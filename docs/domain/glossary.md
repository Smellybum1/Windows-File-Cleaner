...
| Review Shortlist Safety Mix | Read-only summary of shortlisted rows by rating, recommendation, and risk flags. | `ShortlistSafetyMixText` | Review context only. It is not cleanup approval, Quarantine readiness, or storage-savings proof. Tooltip/help text should mirror the current mix and keep no-rescan/no-file-modified/no-readiness-proof/no-savings-proof/not-cleanup-approval boundaries available. |
