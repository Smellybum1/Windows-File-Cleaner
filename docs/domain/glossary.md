...
| Storage Review Search | Read-only text search over completed Storage Scan review rows. | `StorageReviewSearch`, `StorageReviewSearchField` | Broad search matches path, parent path, name, category, rating, recommendation, evidence, Access Status, and access issue text. Field prefixes such as `path:`, `parent:`, `under:`, `category:`, `rating:`, and `access:` restrict matching to one field. |
