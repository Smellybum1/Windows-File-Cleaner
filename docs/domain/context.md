...
- The daily local readiness check may compose accepted-package evidence, accepted launch-command printing, and Restore Manifest Summary output, but it must remain read-only and print-only: no shortcut creation, installer behavior, WPF launch, scan, move, restore, delete, approval, or cleanup history.
