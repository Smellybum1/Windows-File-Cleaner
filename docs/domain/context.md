...
- In WPF, inline Quarantine Preview readiness/status tooltip and automation help text should mirror the status text on both the status text and visible non-clickable `?` help cue, name the current status state, and keep no-create/no-move/no-restore/no-delete/not-cleanup-approval wording available.
...
- Select one discovered Restore Manifest whose Cleanup Scope is exactly `C:\Users\moxhe`, preview selected readiness, preview the selected restore gate, type `RESTORE`, then restore only that selected manifest after immediate revalidation passes.
- Block restore because an original path already exists.
- Block restore because a quarantine path is missing or inaccessible.
- After a restore attempt, tell the user to rediscover manifests and rescan manually before further review.
...
- Requires a fresh Quarantine Manifest Discovery result.
- Requires Selected Restore Manifest Review for one selected manifest.
- Requires fresh Restore Readiness Preview evidence, Selected Restore Confirmation Draft, Selected Restore Execution Gate, and exact `RESTORE` confirmation.
- Revalidates selected restore readiness immediately before calling the restore executor.
- Calls `UndoQuarantineExecutor` for movement and Restore Manifest updates.
- Refuses to overwrite original paths and blocks recovery-review, failed, restoring, not-moved, all-restored, and missing-quarantine-path states.
- Leaves action-folder cleanup, all-manifest restore, and cleanup history deferred.
...
- It may focus the displayed list by exact Cleanup Scope, manifests that need recovery review, or manifests that still have undo work, or fail a terminal check against either the full selected-root state or the currently displayed manifest set when recovery-review or undo-work states remain, but that remains evidence only.
...
### Rule: Portable Release Package stays reversible-only

Portable Release Package is the local self-contained WPF app publish output for running the current v1 without an installer.
...
- The local release launcher may verify and start the latest or selected Portable Release Package, print the command without launching, print a package-level checklist without launching, or write ignored local acceptance notes. It may print package-local `README-FIRST.txt` and launch script paths as guidance, but that is not shortcut installation or cleanup history. Fixture launch mode only prefills the Cleanup Scope; checklist-only mode and acceptance-note writing must not launch WPF, create fixtures, click Scan, move, restore, delete, approve cleanup, or create cleanup history.
- The local release acceptance summary may read ignored `.local\release-acceptance` notes and report checklist/evidence status, but those notes are not app persistence, cleanup history, package metadata, or a substitute for the package verifier.
- The local release acceptance recorder may update ignored `.local\release-acceptance` notes after a human completes package acceptance, but it must not launch WPF, scan, move, restore, delete, approve cleanup, create cleanup history, or turn notes into app persistence.
- The accepted local release launcher may read completed ignored `.local\release-acceptance` notes to choose a previously accepted package, then delegate to local release verification and launch-command printing; this is package selection evidence only, not app persistence, cleanup history, or a current-HEAD package proof.
- The daily local readiness check may compose accepted-package evidence, accepted package verification, accepted launch-command printing, optional Fixture Acceptance Notes summary/completion evidence, and Restore Manifest Summary output, but it must remain read-only and print-only: no shortcut creation, installer behavior, WPF launch, scan, move, restore, delete, approval, or cleanup history. When one wrapper prints multiple accepted launch commands, it may skip duplicate package verification only after the same wrapper run already verified the accepted package successfully.
- The real-profile Quarantine readiness review may compose full MVP preflight, daily local readiness, optional Fixture Acceptance Notes evidence forwarded through daily readiness, and focused Restore Manifest Summary output before a future exact `C:\Users\moxhe` Quarantine batch. Its Restore Manifest display focus defaults to exact `C:\Users\moxhe`, with an explicit all-scope option for fixture-inclusive review, and displayed strictness flags may enforce exact-profile displayed evidence separately from fixture history. Its next-batch evidence preset is exact-profile-only and requires Fixture Acceptance Notes status, displayed Restore Manifest evidence, and zero displayed undo-work manifests before a future tiny exact batch review. It is terminal evidence only: no shortcut creation, installer behavior, WPF launch, real-profile scan, move, restore, delete, approval, cleanup history, or substitute for WPF readiness, exact `QUARANTINE`, Real-Profile Quarantine Approval Evidence, immediate Pre-Execution Revalidation, and explicit user approval for a specific tiny batch.
- The real-profile next-batch checklist may print the manual WPF review steps after the readiness preset, including exact scope, Fixture Acceptance Notes status handling, tiny-batch caps, hard blockers, Quarantine tab evidence, human-only click boundary, and rediscover/rescan follow-up, but it must remain terminal guidance only: no WPF launch, scan, move, restore, delete, manifest write, approval, cleanup history, shortcut creation, or installer behavior. Incomplete Fixture Acceptance Notes should point back to summary guidance; the recorder remains only for after an actual all-pass visible fixture review, while issues or not-checked items should be filled manually.
- The real-profile next-batch review may compose the exact-profile next-batch evidence preset and manual WPF checklist into one terminal-only wrapper, but it must not expose broader Cleanup Scope knobs or become launch automation: no shortcut creation, installer behavior, WPF launch, scan, move, restore, delete, manifest write, approval, cleanup history, or substitute for WPF readiness, exact `QUARANTINE`, Real-Profile Quarantine Approval Evidence, immediate Pre-Execution Revalidation, and explicit user approval for a specific tiny batch.
