...
- Branch: `main`
- Latest completed packet: WPF Pre-Execution Revalidation Evidence
...
The latest code packet wired read-only Pre-Execution Revalidation evidence into WPF Quarantine Preview and Quarantine Execution Gate output. WPF now builds revalidation from the current preview, confirmation draft, action draft, and root-safety evidence when preview readiness exists, passes it into Real-Profile Quarantine Execution Readiness, and shows compact revalidation evidence without enabling real-profile/custom movement. Real-Profile Restore Readiness remains unwired WPF evidence and still blocks real-profile execution.
