...
- Branch: `main`
- Latest completed packet: WPF Execution Bridge Source Guard
...
The latest packet adds a WPF execution bridge source guard that keeps movement executor calls isolated to the three known gated WPF execution methods and verifies those methods remain behind the current gate/fixture checks. This keeps ADR 0017/0018/0019 real-profile boundaries from drifting while real-profile restore and real-profile Quarantine execution remain blocked.
...
4. Decide after manual review whether the compact Quarantine Readiness Summary is sufficient, or whether WPF Execution Readiness still needs a dedicated readiness pane.
...
Please inspect current git status first, then choose the best next small packet. Prefer visible manual fixture review polish, manifest review row layout/readiness clarity, Quarantine Preview/readiness clarity, scan-gate discoverability, or Review Shortlist safety context before any real cleanup execution. Full `.cmd` MVP preflight passed after the selected-restore revalidation checklist alignment. Run the narrowest relevant checks, update docs, and commit/push at clean packet boundaries.
```
