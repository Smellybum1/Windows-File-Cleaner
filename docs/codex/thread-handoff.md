...
- Branch: `main`
- Latest completed packet: Review Mix Help Text
...
1. Run a manual fixture visual polish pass with `.\tools\Start-MvpFixtureReview.ps1 -ChecklistOnly` and then `.\tools\Start-MvpFixtureReview.ps1`; verify the updated prompts match the visible app, including Review Mix / Matched Review Mix tooltip/help text, panel-name collapsed header summaries/state styling and state-naming tooltip/help text, Review Shortlist Safety Mix tooltip/help text, styled inline Quarantine Preview readiness state-naming tooltip/help text, styled Review Grid Mode Status state-naming tooltip/help text, and `Current quarantined` / `Back to scan rows` help text.
