...
- Branch: `main`
- Latest completed packet: Current-Session Quarantined Review
...
1. Manual fixture visual polish pass for the new Quarantine shortlist area and `Quarantined` / `Back to scan rows` flow.
2. Make Quarantine Preview success/readiness harder to miss than a status-bar-only message, especially now that `Preview shortlist quarantine` is the first step before the shortlist-level action.
