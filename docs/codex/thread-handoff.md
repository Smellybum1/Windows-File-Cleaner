...
- `tools\Test-RealProfileSelectedRestoreTrustManifestPathGuard.cmd` covers the sacrificial real-profile selected restore trust helper's action-layout path guard. It runs only with `-WhatIf` and an ignored `.local` Quarantine Root, proving an escaped `..\moxhe\...` relative path fails before preview output and without writing Restore Manifests or modifying real-profile files.
