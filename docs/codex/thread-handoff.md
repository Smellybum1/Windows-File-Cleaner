...
- Latest tooling/evidence packet: `2026-06-04-local-release-path-guard-regression`
...
- `tools\Test-LocalReleasePathGuards.cmd` covers portable release verifier and launcher path guards. MVP preflight now runs it by default before local release acceptance command stamping. It asserts explicit non-`.local` release roots and release paths fail before verifier, launcher, or launch-command output.
