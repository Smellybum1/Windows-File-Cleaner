...
- Branch: `main`
- Latest completed packet: Full Local MVP Preflight After Header Help
...
1. Run a manual fixture visual polish pass with `.\tools\Start-MvpFixtureReview.ps1 -ChecklistOnly` and then `.\tools\Start-MvpFixtureReview.ps1`; verify the updated prompts match the visible app, including styled inline Quarantine Preview readiness, styled Review Grid Mode Status, and `Quarantined` / `Back to scan rows` help text.
2. Decide whether a future `Quarantined` view should include discovered Restore Manifest entries or stay current-session-only.
