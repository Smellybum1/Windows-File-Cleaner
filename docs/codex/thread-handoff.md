...
- Latest tooling/evidence packet: `2026-06-04-real-profile-selected-restore-trust-helper-path-guard`
...
- `tools\Test-RealProfileSelectedRestoreTrustManifestPathGuard.cmd` covers the sacrificial real-profile selected restore trust helper's root and action-layout path guards. It runs only with `-WhatIf`, an ignored `.local` Quarantine Root, and committed `README.md` as a non-`.local` rejection target, proving out-of-bound roots and escaped `..\moxhe\...` relative paths fail before preview output and without writing Restore Manifests or modifying real-profile files.
