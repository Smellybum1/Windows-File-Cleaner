...
- `tools\Test-RealProfileSelectedRestoreTrustManifestPathGuard.cmd` verifies the sacrificial real-profile selected restore trust helper keeps generated quarantine source paths under the action `items` root. The regression runs only with `-WhatIf` and an ignored `.local` Quarantine Root, so it does not write Restore Manifests or modify real-profile files.
