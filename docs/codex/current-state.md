...
- Latest tooling/evidence packet: daily readiness exact profile stop action.
- Latest docs/workflow packet: closeout startup compaction.
...
- `tools\Test-RealProfileSelectedRestoreTrustManifestPathGuard.cmd` verifies the sacrificial real-profile selected restore trust helper keeps explicit roots under the default `D:\WindowsFileCleanerQuarantine` root or ignored `.local`, and keeps generated quarantine source paths under the action `items` root. The regression now derives its escaped profile segment from the helper's safe preview output, uses a `-WhatIf`-only non-`moxhe` override for clean runners, proves that override is rejected without `-WhatIf`, and MVP preflight runs it by default. It uses an ignored `.local` Quarantine Root and committed `README.md` as a non-`.local` rejection target, so it does not write Restore Manifests or modify real-profile files.
...
- Completed fixture acceptance summaries now say fixture acceptance evidence is complete and no recorder action is pending.
- `tools\Test-FixtureRootPathGuard.cmd` verifies `New-StorageScanSmokeFixture.cmd` and `Start-MvpFixtureReview.cmd` reject explicit fixture roots outside ignored `.local` before fixture writes, checklist output, or WPF launch. MVP preflight runs it by default before the fixture dry run; use `-SkipFixtureRootPathGuardCheck` only for focused local loops.
- `tools\Test-FixtureAcceptanceNotes.cmd` verifies fixture acceptance notes summary and recorder behavior with temporary ignored notes: incomplete summaries show recording guidance, `-RequireComplete` fails with blockers, explicit summary paths outside ignored `.local` are rejected before output or recorder guidance, recorder calls require `-RecordManualAcceptance`, `-WhatIf` does not write, and explicit recording completes synthetic notes.
...
- `tools\Test-DailyReadinessFixtureAcceptanceNotes.cmd` verifies optional and strict daily readiness fixture-note forwarding with temporary ignored synthetic package files, package acceptance notes, fixture acceptance notes, and an empty Restore Manifest root. Incomplete required fixture notes fail before accepted launch-command printing; explicit fixture notes paths outside ignored `.local` fail before accepted launch-command printing; optional incomplete notes show recording guidance and continue; complete required notes pass the normal print-only daily readiness flow.
...
- `tools\Test-DailyReadinessLatestPackageNotes.cmd` verifies daily readiness keeps accepted evidence on completed notes while showing an explicit incomplete candidate notes file as informational context with guarded next steps, including `-RecordCommitMismatch`; it verifies explicit package acceptance notes paths outside ignored `.local` fail before latest-notes or launch-command printing; it also verifies an explicit malformed-looking notes file reports missing evidence as informational context and still cannot block accepted-package readiness. The regression now keeps its synthetic notes under its private ignored `.local\daily-readiness-latest-package-notes-test` folder instead of the default `.local\release-acceptance` search root.
...
- `tools\Test-LocalReleasePathGuards.cmd` verifies `Publish-LocalRelease.cmd`, `Test-LocalRelease.cmd`, and `Start-LocalRelease.cmd` reject explicit release roots and release paths outside ignored `.local` before publisher, verifier, launcher, or launch-command output. MVP preflight runs it by default before the local release acceptance command stamping regression; use `-SkipLocalReleasePathGuardCheck` only for focused local loops.
- `tools\Test-LocalReleaseAcceptanceCommandStamping.cmd` verifies generated package acceptance notes stamp the actual `-ReleasePath` verifier/checklist commands, include `-RequireCurrentCommit` only when that switch created the notes, and print the guarded `-RecordCommitMismatch` recorder command only for behind-current-`HEAD` notes. It uses temporary synthetic release folders and generated notes under ignored `.local`.
- MVP preflight now runs the local release acceptance command stamping regression by default before the accepted local release selection regression; use `-SkipLocalReleaseAcceptanceCommandStampingCheck` only for focused local loops.
- `tools\Test-AcceptedLocalReleaseSelection.cmd` verifies that accepted-package launch commands ignore newer incomplete and malformed-looking notes when selecting by default, while explicit incomplete or malformed-looking notes paths stop before launch-command printing. It also verifies explicit accepted launcher notes paths outside ignored `.local` fail before accepted launcher output or launch-command printing. It uses temporary ignored notes and synthetic package files under `.local` with `-PrintOnly -SkipVerify`, and uses committed `README.md` only as a non-`.local` rejection target.
...
- Push CI run #391 for commit `9204247` passed, validating the current normal push path after the daily readiness stop-action reminder joined default MVP preflight coverage. Earlier #389 validated exact skip-switch documentation consistency coverage, earlier #387 validated initial skip-switch documentation consistency coverage, earlier #385 validated trust-helper path guard preflight coverage, earlier #365 validated active feature-index documentation consistency coverage, and earlier #360 validated the normal push fallback for `inputs.runner_image || 'windows-2022'`.
...
- Current full MVP preflight passed with restore, build, core tests, WPF app tests, fixture root path guard regression, fixture `-WhatIf`, fixture checklist-only output, fixture acceptance notes regression, daily readiness fixture acceptance notes regression, daily readiness latest package notes regression, local release path guard regression, local release acceptance command stamping regression, accepted local release selection regression, local release acceptance summary regression, local release acceptance recorder regression, real-profile selected restore trust helper path guard regression, real-profile next-batch stop guard regression, daily readiness exact-profile undo spotlight regression, documentation consistency regression, and whitespace diff. A fresh CI-shaped `D:\a\Windows-File-Cleaner\Windows-File-Cleaner` checkout also passed the targeted package summary regression and full MVP preflight after the runner-path wrapping assertion fix.
...
- Daily readiness now prints an exact-profile undo-work stop-state spotlight after the broad Restore Manifest summary, followed by a next-action reminder that nonzero displayed exact-profile undo work blocks next-batch movement evidence. On current evidence it shows one exact-profile undo-work manifest: `quarantine-action-draft-20260604014901-b7b402a2`.
- `tools\Test-DailyReadinessExactProfileUndoSpotlight.cmd` verifies with temporary ignored synthetic Restore Manifests that broad daily readiness can show fixture and exact-profile undo work while the final spotlight displays only exact-profile undo work and prints the stop-state action reminder. The regression uses `Invoke-DailyLocalReadiness.cmd -SyntheticRestoreManifestOnly` so CI and clean runners do not need ignored accepted-package notes or package folders for this focused check. It now also asserts synthetic daily readiness rejects missing or outside-`.local` Quarantine Roots and package/fixture acceptance parameters.
