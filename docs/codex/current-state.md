...
- `tools\Test-LocalReleasePathGuards.cmd` verifies `Test-LocalRelease.cmd` and `Start-LocalRelease.cmd` reject explicit release roots and release paths outside ignored `.local` before verifier, launcher, or launch-command output. MVP preflight runs it by default before the local release acceptance command stamping regression; use `-SkipLocalReleasePathGuardCheck` only for focused local loops.
