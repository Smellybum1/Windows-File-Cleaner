...
- Latest tooling/evidence packet: daily readiness latest package notes regression.
...
- `tools\Test-DailyReadinessLatestPackageNotes.cmd` verifies daily readiness keeps accepted evidence on completed notes while showing a newer incomplete candidate notes file as informational context with guarded next steps, including `-RecordCommitMismatch`.
...
- `tools\Test-AcceptedLocalReleaseSelection.cmd` verifies that accepted-package launch commands ignore newer incomplete notes when selecting by default, while an explicit incomplete notes path stops before launch-command printing. It uses temporary ignored notes and synthetic package files under `.local` with `-PrintOnly -SkipVerify`.
