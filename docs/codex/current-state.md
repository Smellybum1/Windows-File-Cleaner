...
- `tools\Test-RealProfileSelectedRestoreTrustManifestPathGuard.cmd` verifies the sacrificial real-profile selected restore trust helper keeps explicit roots under the default `D:\WindowsFileCleanerQuarantine` root or ignored `.local`, and keeps generated quarantine source paths under the action `items` root. The regression now derives its escaped profile segment from the helper's safe preview output and MVP preflight runs it by default. It runs only with `-WhatIf`, an ignored `.local` Quarantine Root, and committed `README.md` as a non-`.local` rejection target, so it does not write Restore Manifests or modify real-profile files.
