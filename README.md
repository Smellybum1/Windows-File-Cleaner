...
- ADR 0019 records the Real-Profile Selected Restore Execution contract. The current WPF app implements that selected-manifest-only restore path for exact `C:\Users\moxhe` Restore Manifests, while real-profile Quarantine execution remains unavailable.
...
Those workflows require the Real-Profile Quarantine Readiness Contract from ADR 0017 and ADR 0018: exact confirmation semantics, approval evidence, immediate revalidation, Quarantine Root safety checks, trusted selected-manifest Undo Quarantine/recovery behavior, tests, and explicit approval before the visible app can quarantine real-profile files.
