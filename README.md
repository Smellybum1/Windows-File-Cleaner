...
- The visible WPF app can move files when the Cleanup Scope is a recognized synthetic fixture and the exact Quarantine confirmation gate is open.
- The visible WPF app can move files from the exact real-profile Cleanup Scope `C:\Users\moxhe` only for the first ADR 0018 phase after all readiness evidence, exact `QUARANTINE`, Real-Profile Quarantine Approval Evidence, and immediate pre-execution revalidation pass. Codex and automated tests do not click this real-profile movement path.
