...
For ordinary local review, start from the accepted portable package and keep daily readiness evidence terminal-only:
...
Before considering another tiny exact real-profile Quarantine batch, run the terminal-only next-batch review wrapper and stop for human review:
...
Detailed daily command behavior lives in `docs/operations/daily-use.md`, accepted package commands in `docs/operations/portable-release.md`, and Restore Manifest evidence review in `docs/operations/restore-manifest-review.md`.

All commands above are evidence or launch-command helpers unless `-PrintOnly` is intentionally removed. They do not install shortcuts, click `Scan`, scan, move, restore, delete, approve cleanup, or create cleanup history. Stop before any real-profile Quarantine or selected restore execution unless the specific batch or selected Restore Manifest has fresh readiness evidence, exact confirmation, and explicit user approval.
...
- `.\tools\Show-RealProfileNextBatchChecklist.cmd` prints the manual WPF checklist for a future tiny exact real-profile batch and points to the combined review wrapper without launching WPF, scanning, moving, restoring, deleting, approving cleanup, or creating cleanup history.
...
Before launching, the script prints the fixture review checklist. Treat that generated checklist as the current source of truth; the README only names the workflow and safety boundary.

The fixture checklist includes final real-profile/custom blocker checks, but those are not instructions to scan `C:\Users\moxhe` during the fixture pass. Use custom preview-only paths or existing synthetic real-profile readiness coverage unless you intentionally start a separate real-profile retest after MVP preflight.

See `docs/operations/manual-fixture-review.md` for the checklist-only and acceptance-notes workflow.
...
The template is written under `.local\fixture-review-acceptance`, stamps the repo path, Git branch/commit, worktree status at notes creation, .NET SDK, WPF app project/target framework/WPF flag, required preflight command, local evidence checkboxes, and exact post-pass recorder and summary commands, then gives each grouped checklist item pass/issue/not-checked slots. Use `.\tools\Start-MvpFixtureReview.cmd -SkipPreflight -WriteAcceptanceNotes` after a successful preflight when you want the same notes template created before the visible fixture app launches. When notes are written, the launcher also prints exact follow-up recorder, summary, and `-RequireComplete` completion-check commands for that notes file.
...
Or use an explicit notes file:

```powershell
.\tools\Record-FixtureAcceptanceNotes.cmd -Path ".local\fixture-review-acceptance\fixture-acceptance-YYYYMMDD-HHMMSS.md" -RecordManualAcceptance
.\tools\Summarize-FixtureAcceptanceNotes.cmd -Path ".local\fixture-review-acceptance\fixture-acceptance-YYYYMMDD-HHMMSS.md" -RequireComplete
```

`Record-FixtureAcceptanceNotes` updates ignored `.local` notes only. Use it only after the visible fixture pass, read-only fixture Scan, fixture Quarantine/undo review, Restore Manifest review, selected restore review, and real-profile/custom blocker checks have actually been manually confirmed; it does not launch WPF, create fixtures, scan, move, restore, delete, approve cleanup, or create cleanup history.
...
The automated `WindowsFileCleaner.App.Tests` project scans a synthetic fixture through the WPF shell and covers the main read-only review, fixture Quarantine, selected restore, all-manifest readiness, real-profile readiness evidence, tooltip/help-text, and wrapping-layout surfaces. It does not replace checking the visible layout and controls by eye.
...
To focus the displayed manifest list to the exact real-profile Cleanup Scope:
...
For the full fixture UI acceptance checklist, use the generated checklist from `.\tools\Start-MvpFixtureReview.cmd -ChecklistOnly`; the supporting runbook is `docs/operations/manual-fixture-review.md`. The README keeps only the stable manual review flow.
...
9. Use Review Mix, Matched Review Mix, and Safety Summary to inspect safety signals and active filters. Their `?` help cues should keep the read-only/no-rescan/no-file-modified/not-cleanup-approval boundary visible, and Safety Summary shortcuts should land back on Main Grid after applying a review lens.
...
21. Confirm the Safety Summary, Review, Quarantine, and Main Grid tabs remain readable and keep their `?` help cues paired with the related control. For the exhaustive tab, gate, tooltip, preview, and selected-restore acceptance checks, use `.\tools\Start-MvpFixtureReview.cmd -ChecklistOnly`.
...
23. On that same fixture execution, use `Current quarantined (N)` and `Back to scan rows` to switch between current-session moved entries and normal Storage Scan rows without rescanning. Then click `Undo fixture quarantine` to restore the synthetic entry, update the Restore Manifest, disable repeat undo, and keep stale-state wording visible.
...
26. Use `Preview selected restore gate`, then type `RESTORE`. Fixture selected restore can execute after the gate opens; exact real-profile selected restore can open only after Selected Restore Pre-Execution Revalidation passes, and Codex should not click real-profile restore unless the user explicitly chooses that specific movement test.
...
28. On custom non-fixture or non-exact real-profile Cleanup Scopes, typing `QUARANTINE` should still leave execution disabled with a scope-specific blocker and no undo action. During fixture review, do not scan `C:\Users\moxhe` just to satisfy this check; use custom preview-only paths or existing synthetic readiness evidence unless you intentionally start a separate real-profile retest.
