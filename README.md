...
- The WPF header uses compact scan totals plus a wrapping status strip for the Cleanup Scope Safety Note, scan-gate summary, and scan-gate detail so wide windows use horizontal space before consuming review height.
- The WPF work surface uses horizontal tabs for Safety Summary, Review, Quarantine, and Main Grid so the dense review and readiness sections each have their own page while the Cleanup Scope, scan totals, and scan-gate header stay visible.
...
6. Review the compact header scan totals for total size, folders, files, and access issues.
