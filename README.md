...
- Safety Summary is collapsible, its header summarizes compact risk counts, header tooltip/help text mirrors the summary, and its review shortcuts use disabled-state tooltips and automation help text to keep read-only shortcut scope, no-rescan, no-file-modified, no-permission-change, no-link-following, and not-cleanup-approval boundaries available.
...
- Quarantine Preview is a dry run only, and its semantically styled inline status plus preview/gate panes keep Review Shortlist and Quarantine Preview separate from cleanup approval.
...
- Quarantine Root Selection, Quarantine Preview, inline preview readiness, Quarantine Execution Gate, fixture execution, and current-fixture undo are grouped in a collapsible Quarantine shortlist area above the main grid; its header summarizes shortlist, preview, current quarantined, and undo state, header tooltip/help text mirrors the summary, while verbose gate details stay in a constrained scroll area so the review grid remains usable.
...
- Review Grid Mode Status labels whether the main grid is showing Storage Scan rows or current-session quarantined items, uses lightweight styling for neutral/informational/warning states, points back to `Back to scan rows`, and warns when scan rows may be stale after fixture Quarantine execution.
- `Quarantined` and `Back to scan rows` tooltips/help text explain disabled states, current-session scope, read-only behavior, discovery for older manifests, and that returning to scan rows does not rescan or undo.
...
11. Check or browse the Quarantine root and generate Quarantine Preview for Review Shortlist readiness review; the styled inline preview readiness line plus browse and preview/export tooltips should keep preview-only, no-folder-creation, dry-run, report-only, and not-cleanup-approval boundaries visible.
12. For fixture scopes, optionally type `QUARANTINE` once, run `Quarantine included shortlist` for all included Review Shortlist rows, use `Quarantined` / `Back to scan rows` to review moved current-session entries, then use `Undo fixture quarantine` before or after a rescan to prove the reversible visible workflow.
