...
- The WPF header uses compact scan totals, compact Review Shortlist totals, and a wrapping status strip for the Cleanup Scope Safety Note, scan-gate summary, and scan-gate detail so wide windows use horizontal space before consuming review height.
- The WPF work surface uses horizontal tabs for Safety Summary, Review, Quarantine, and Main Grid so the dense review and readiness sections each have their own page while the Cleanup Scope, scan totals, Review Shortlist totals, and scan-gate header stay visible.
...
- Quarantine Preview, Quarantine Readiness Summary, and Quarantine Execution Gate show a read-only Execution Readiness contract that names fixture-executable, real-profile-candidate, and custom-preview-only states, includes read-only Quarantine Root Execution Safety, non-`D:` root acknowledgement evidence, Pre-Execution Revalidation, and Real-Profile Restore Readiness evidence when supporting evidence exists, and keeps grouped missing readiness dimensions visible without enabling real-profile/custom movement.
...
- Core Real-Profile Quarantine Approval Evidence records that exact `QUARANTINE` is necessary but not sufficient; current-build real-profile movement availability still defaults to unavailable and the model remains read-only. WPF shows this evidence in the Quarantine Execution Gate for real-profile/custom preview-only scopes without enabling movement.
...
- Quarantine Root Selection, non-`D:` root readiness acknowledgement with a visible hoverable `?` help cue, Quarantine Preview, inline preview readiness, compact Quarantine Readiness Summary with concise key-blocker labels, Quarantine Execution Gate, fixture execution, and current-fixture undo are grouped in the dedicated Quarantine tab; its header starts with the visible panel name, summarizes shortlist, preview, current quarantined, and undo state with lightweight semantic styling, header tooltip/help text mirrors the summary and names the current header state, its visible `?` help cue mirrors the same header help text, while verbose gate details stay in a constrained scroll area so the Main Grid tab remains usable.
...
- ADR 0019 records the Real-Profile Selected Restore Execution contract for a future selected-manifest-only real-profile restore path. The current WPF app still keeps selected real-profile restore unavailable.
- For exact real-profile selected Restore Manifests, the Selected Restore Execution Gate shows read-only Selected Restore Pre-Execution Revalidation evidence, including stale missing-quarantine-path blockers, while keeping real-profile selected restore unavailable.
...
6. Review the compact header scan totals for total size, folders, files, and access issues, plus the compact Review Shortlist totals for shortlisted size, folders, and files. The shortlist size is row-size review context, not storage savings or cleanup approval.
...
28. On `C:\Users\moxhe` or a custom non-fixture Cleanup Scope, typing `QUARANTINE` should still leave `Quarantine included shortlist` disabled with a scope-specific blocker and no undo action. During the fixture pass, do not scan `C:\Users\moxhe` just to satisfy this check; use custom preview-only paths or existing synthetic real-profile readiness evidence unless you intentionally start a separate real-profile retest after MVP preflight. The Quarantine Execution Gate should show read-only Real-Profile Quarantine Approval Evidence with `Can approve real-profile movement: no`. The blocker should keep ADR 0017 visible: Review Shortlist, exact `QUARANTINE`, a clean preview, and real-profile scan acknowledgement are not enough to move real-profile files until immediate revalidation, Quarantine Root safety, trusted Undo/recovery, and approval semantics are designed. ADR 0018 first-phase blockers such as 10 rows, 1 GB, no-category rows, and strict descendant checks should stay visible when they apply. Selected restore should also keep ADR 0019 visible by showing preview-only scope status and staying unavailable for non-fixture manifests even when `RESTORE` is typed and selected restore revalidation evidence is clean.
...
17. For real-profile scopes, confirm `Quarantine included shortlist` and broad Undo stay unavailable, with read-only Real-Profile Quarantine Approval Evidence and ADR 0017 readiness-contract blockers visible before any real-profile cleanup execution.
...
Those workflows require the Real-Profile Quarantine Readiness Contract from ADR 0017 and the selected real-profile restore contract from ADR 0019: separate design, exact confirmation semantics, approval evidence, immediate revalidation, Quarantine Root safety checks, trusted selected-manifest Undo Quarantine/recovery behavior, tests, and explicit approval before the visible app can move or restore real-profile files.
