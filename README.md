...
- Quarantine Preview is a dry run only, and its inline status plus preview/gate panes keep Review Shortlist and Quarantine Preview separate from cleanup approval.
...
- Review Grid Mode Status labels whether the main grid is showing Storage Scan rows or current-session quarantined items, points back to `Back to scan rows`, and warns when scan rows may be stale after fixture Quarantine execution.
...
11. Check or browse the Quarantine root and generate Quarantine Preview for Review Shortlist readiness review; the inline preview readiness line plus browse and preview/export tooltips should keep preview-only, no-folder-creation, dry-run, report-only, and not-cleanup-approval boundaries visible.
